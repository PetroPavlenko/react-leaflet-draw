...
var EditControl = function (_LayersControl) {
  ...
  function EditControl() {
    ...
    }, _this._updateEvents = function (type) {
      var changeEvent = _this.context.map[type].bind(_this.context.map);

      changeEvent('draw:created', _this.onDrawCreate);

      for (var key in eventHandlers) {
        if (_this.props[key]) {
          changeEvent(eventHandlers[key], _this.props[key]);
        }
      }
...
EditControl.propTypes = _extends({}, Object.keys(eventHandlers).reduce(function (acc, val) {
  ...
  otherEvents: _propTypes.PropTypes.arrayOf(_propTypes.PropTypes.shape({
